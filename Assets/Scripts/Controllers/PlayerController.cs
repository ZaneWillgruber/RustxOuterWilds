...
public class PlayerController : GravityObject
{
    ...
    void Update()
    {
        if (referenceBody)
        {
            if (referenceBody.gravityField.bounds.Contains(rb.position))
            {
                rotationAffected = true;
                HandleMovement();
            }
            else
            {
                rotationAffected = false;
                HandleSpaceMovement();
            }
        }
        else
    ...
    void HandleSpaceMovement()
    {
        ...
        float smoothPitchOld = smoothPitch;
        smoothPitch = Mathf.SmoothDampAngle(smoothPitch, pitch, ref pitchSmoothV, rotationSmoothTime);
        float smoothYawOld = smoothYaw;
        smoothYaw = Mathf.SmoothDampAngle(smoothYaw, yaw, ref yawSmoothV, rotationSmoothTime);
        transform.Rotate(Vector3.right * Mathf.DeltaAngle(smoothPitchOld, smoothPitch), Space.Self);
    ...
    void FixedUpdate()
    {
        ...
        //if in bounds of reference body gravity field collider
        if (rotationAffected)
        {
        ...
        // Adjust velocity
        // Vector3 targetPosition = rb.position + smoothVelocity * Time.fixedDeltaTime;
